...
class Authorize_net
{
    ...
    private $response = '';				// Response from Authorize.net
		...
		private $paid_amount = '';		// The amount paid from Authorize.net
		private $payment_status = '';		// The paid status from Authorize.net
		private $approval_code = '';		// The approval code from Authorize.net
		private $msg = '';		// The description from Authorize.net
	...
	public function getRefundVals()
	{
		$refund_net_vals = array(
			'x_login'				=> $this->api_login_id,
			'x_tran_key'			=> $this->api_transaction_key,
			'x_version'				=> '3.1',
			'x_delim_char'			=> '|',
		...
		return array_merge($refund_net_vals, $this->post_vals);
	}
	...
	public function parseResponse( $response )
	{
		...
		elseif( is_string($response) && strpos($response, '|') !== FALSE )
		{
			...
			if( isset($res[0]) )
			{
				...
				{
					case '1': // Approved
					$this->payment_status = isset($res[0]) ? $res[0] : '';
					$this->transation_id = isset($res[6]) ? $res[6] : '';
					$this->approval_code = isset($res[4]) ? $res[4] : '';
					$this->paid_amount		 = isset($res[9]) ? $res[9] : '';
					$this->msg = 'This transaction has been approved.';
					return TRUE;
					break;
					...
					if( isset($res[3]) )
					{
						if($res[3] === 54)
						{
							$this->msg = 'This payment is still being processed and can not be refunded yet.Please try again later.';
						}
						$this->payment_status = $res[3];
						$this->error = $res[3];
					}
					return TRUE;
					break;
	...
	function parseRefundResponse($resp = '') {
        ...
        try
        {
            ...
            {
                case '1':
                    $output['message'] = "Refund initiated successfully.";
                break;
                case '2':
                case '3':
                case '4':                   
                    $msg = "This payment is still being processed and can not be refunded yet.Please try again later.";
